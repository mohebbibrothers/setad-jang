...
def _funded_campaign(*, amount: int = 100_000):
    ...
    campaign.save(
        update_fields=["purchased_amount", "purchased_shares", "participant_count", "updated_at"]
    )
    return campaign
...
def test_admin_disbursement_api_full_workflow_with_audit() -> None:
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        approve_response = client.post(
            reverse(
                "madadkar:admin-disbursement-action",
                kwargs={"disbursement_id": disbursement_id, "action": "approve"},
            ),
        )
        paid_response = client.post(
            reverse(
                "madadkar:admin-disbursement-action",
                kwargs={"disbursement_id": disbursement_id, "action": "mark-paid"},
            ),
...
def test_admin_disbursement_list_detail_and_disbursable_endpoint() -> None:
    ...
    list_response = client.get(
        reverse("madadkar:admin-disbursement-list-create"), data={"campaign": campaign.pk}
    )
    detail_response = client.get(
        reverse("madadkar:admin-disbursement-detail", kwargs={"disbursement_id": disbursement.pk})
    )
    summary_response = client.get(
        reverse("madadkar:admin-campaign-disbursable", kwargs={"campaign_id": campaign.pk})
    )
