...
def test_user_activity_timeline_api_is_owner_scoped_and_filterable() -> None:
    ...
    user = UserFactory()
    other = UserFactory()
    record_activity(user=user, event_type="madadkar.payment_success", title="پرداخت موفق", app_label="madadkar")
    record_activity(user=other, event_type="support.reply", title="پاسخ", app_label="support_desk")

    response = _client_for(user).get(reverse("activity:user-timeline"), data={"app_label": "madadkar"})
