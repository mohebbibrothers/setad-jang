...
class TestSupportUserTicketAPI:
    ...
    def test_user_can_create_update_submit_and_filter_own_ticket(self) -> None:
        ...
        update_response = client.patch(
            ...
            data={"ticket_type_id": ticket_type.pk, "subject": "مشکل پرداخت مددکار", "description": "پرداخت انجام شده ولی ثبت نشده است"},
            format="json",
        )
        assert update_response.status_code == status.HTTP_200_OK
        assert update_response.data["data"]["subject"] == "مشکل پرداخت مددکار"

        submit_response = client.post(reverse("support_desk:user-ticket-submit", kwargs={"ticket_number": ticket_number}))
        ...
        list_response = client.get(reverse("support_desk:user-ticket-list-create"), data={"search": "مددکار"})
    ...
    def test_user_cannot_access_other_user_ticket(self) -> None:
        ...
        response = _client_for(other).get(reverse("support_desk:user-ticket-detail", kwargs={"ticket_number": ticket.ticket_number}))
    ...
    def test_submitted_ticket_cannot_be_edited_by_user(self) -> None:
        ...
        response = _client_for(ticket.owner).patch(
            reverse("support_desk:user-ticket-detail", kwargs={"ticket_number": ticket.ticket_number}),
            data={"ticket_type_id": ticket.ticket_type_id, "subject": "ویرایش غیرمجاز", "description": "بعد از ارسال نباید ویرایش شود"},
            format="json",
        )
...
class TestSupportConversationTimelineAndAttachmentsAPI:
    ...
    def test_user_reply_and_timeline_never_expose_internal_notes(self) -> None:
        ...
        add_internal_note(ticket=ticket, admin=admin, body="این یادداشت نباید به کاربر نمایش داده شود")

        reply_response = _client_for(ticket.owner).post(
            reverse("support_desk:user-ticket-reply", kwargs={"ticket_number": ticket.ticket_number}),
            data={"body": "شماره پیگیری ۱۲۳۴"},
            format="json",
        )
        timeline_response = _client_for(ticket.owner).get(reverse("support_desk:user-ticket-timeline", kwargs={"ticket_number": ticket.ticket_number}))
    ...
    def test_user_can_upload_public_attachment(self) -> None:
        ...
        with patch(_AUDIT_TASK_PATH) as mock_task:
            ...
            response = _client_for(ticket.owner).post(
                reverse("support_desk:user-ticket-attachment", kwargs={"ticket_number": ticket.ticket_number}),
...
class TestSupportSuggestReopenAndSatisfactionAPI:
    ...
    def test_user_can_rate_and_reopen_resolved_ticket(self) -> None:
        ...
        satisfaction_response = _client_for(ticket.owner).post(
            reverse("support_desk:user-ticket-satisfaction", kwargs={"ticket_number": ticket.ticket_number}),
        ...
        reopen_response = _client_for(ticket.owner).post(
            reverse("support_desk:user-ticket-reopen", kwargs={"ticket_number": ticket.ticket_number}),
    ...
    def test_user_cannot_rate_unresolved_ticket(self) -> None:
        ...
        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert SupportTicketMessage.objects.filter(ticket=ticket, message_type="internal_note").count() == 0
