...
def _make_enrollment(
    *, course, user, status_value=EnrollmentStatus.ACTIVE, progress=Decimal("0.00")
):
...
class TestLMSCourseAnalytics:
    ...
    def test_admin_course_analytics_returns_counts_and_average_score(self) -> None:
        ...
        enrollment_a = _make_enrollment(
            course=course,
            user=user_a,
            status_value=EnrollmentStatus.COMPLETED,
            progress=Decimal("100.00"),
        )
        enrollment_b = _make_enrollment(
            course=course,
            user=user_b,
        ...
        attempt_a = _make_attempt(
            course=course, enrollment=enrollment_a, user=user_a, score=Decimal("18.00"), passed=True
        )
        _make_attempt(
            course=course,
        ...
        response = _client_for(admin).get(
            reverse("lms:admin-course-analytics", kwargs={"course_id": course.pk})
        )
    ...
    def test_admin_course_leaderboard_orders_by_score_and_badge(self) -> None:
        ...
        winner_enrollment = _make_enrollment(
            course=course,
            user=winner,
            status_value=EnrollmentStatus.COMPLETED,
            progress=Decimal("100.00"),
        )
        runner_enrollment = _make_enrollment(
            course=course,
            user=runner,
            status_value=EnrollmentStatus.COMPLETED,
            progress=Decimal("100.00"),
        )
        winner_attempt = _make_attempt(
            course=course,
            enrollment=winner_enrollment,
            user=winner,
            score=Decimal("19.50"),
            passed=True,
        )
        runner_attempt = _make_attempt(
            course=course,
            enrollment=runner_enrollment,
            user=runner,
            score=Decimal("17.00"),
            passed=True,
        )
        ...
        response = _client_for(admin).get(
            reverse("lms:admin-course-leaderboard", kwargs={"course_id": course.pk})
        )
...
class TestLMSCourseExport:
    ...
    def test_admin_course_export_returns_xlsx_and_dispatches_audit(self) -> None:
        ...
        with patch(_AUDIT_TASK_PATH) as mock_task:
            ...
            response = _client_for(admin).get(
                reverse("lms:admin-course-export", kwargs={"course_id": course.pk})
            )
        ...
        assert (
            mock_task.delay.call_args.kwargs["action"] == audit_actions.LMS_COURSE_REPORT_EXPORTED
        )
