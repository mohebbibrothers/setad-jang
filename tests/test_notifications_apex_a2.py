...
class TestNotificationServiceContracts:
    ...
    def test_mark_read_and_mark_all_read_are_owner_safe(self) -> None:
        ...
        event2 = create_notification_event(event_type="x", recipients=[user], payload={"title": "x"})
    ...
    def test_dispatch_task_processes_event(self) -> None:
        event = create_notification_event(event_type="task.event", recipients=[UserFactory()], payload={"title": "Task"})
...
class TestNotificationAPIs:
    ...
    def test_user_inbox_mark_read_preferences_and_admin_lists(self) -> None:
        ...
        event = create_notification_event(event_type="api.event", recipients=[user], payload={"title": "اعلان", "message": "متن"})
        dispatch_event(event=event)
        delivery = event.deliveries.get()
        ...
        preference = client.post(
            ...
            data={"event_type": "api.event", "channel": NotificationChannel.EMAIL, "enabled": False},
            format="json",
        )
        assert preference.status_code == status.HTTP_201_CREATED
        assert NotificationPreference.objects.filter(user=user, event_type="api.event", channel=NotificationChannel.EMAIL, enabled=False).exists()

        assert _client_for(admin).get(reverse("notifications:admin-event-list")).status_code == status.HTTP_200_OK
        assert _client_for(admin).get(reverse("notifications:admin-delivery-list")).status_code == status.HTTP_200_OK
        assert _client_for(admin).get(reverse("notifications:admin-template-list")).status_code == status.HTTP_200_OK
    ...
    def test_mark_all_read_endpoint(self) -> None:
        user = UserFactory()
        event = create_notification_event(event_type="bulk", recipients=[user], payload={"title": "Bulk"})
        dispatch_event(event=event)
