...
def test_lesson_progress_records_progressed_and_completed_statement() -> None:
    ...
    verbs = list(LearningActivityStatement.objects.filter(actor=user, lesson=lesson).values_list("verb", flat=True))
...
def test_quiz_start_submit_and_certificate_issue_emit_statements() -> None:
    ...
    verbs = set(LearningActivityStatement.objects.filter(actor=user, course=course).values_list("verb", flat=True))
...
def test_admin_learning_activity_statement_list_endpoint() -> None:
    ...
    response = _client(admin).get(reverse("lms:admin-activity-statement-list"), data={"verb": LearningStatementVerb.INITIALIZED})
