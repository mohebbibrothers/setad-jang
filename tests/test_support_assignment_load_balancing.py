...
def test_assignment_recommendation_prefers_least_loaded_agent() -> None:
    ...
    assert (
        recommendation.candidates[0].workload_score < recommendation.candidates[-1].workload_score
    )
...
def test_assignment_recommendation_endpoint_audits_sensitive_operational_read() -> None:
    ...
    ticket = _open_ticket(
        department=SupportDepartmentFactory(), assignee=None, status_value=TicketStatus.SUBMITTED
    )
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        response = client.get(
            reverse(
                "support_desk:admin-ticket-assignment-recommendation",
...
def test_auto_assign_endpoint_applies_recommendation_and_audits_assignment() -> None:
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        response = client.post(
            reverse(
                "support_desk:admin-ticket-auto-assign",
                kwargs={"ticket_number": ticket.ticket_number},
            ),
