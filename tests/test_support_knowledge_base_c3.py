...
def test_public_knowledge_article_list_detail_and_recommend_endpoint() -> None:
    ...
    list_response = client.get(reverse("support_desk:knowledge-article-list"), data={"search": "آپلود"})
    detail_response = client.get(reverse("support_desk:knowledge-article-detail", kwargs={"slug": article.slug}))
    recommend_response = client.post(
        reverse("support_desk:knowledge-article-recommend"),
        data={"subject": "مشکل آپلود", "description": "آپلود مدرک انجام نمی‌شود", "department_id": department.pk},
        format="json",
    )
...
def test_admin_knowledge_article_lifecycle_and_audit() -> None:
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        update_response = client.patch(
            reverse("support_desk:admin-knowledge-article-detail", kwargs={"article_id": article_id}),
            data={"summary": "توضیح به‌روز SLA"},
            format="json",
        )
        publish_response = client.post(reverse("support_desk:admin-knowledge-article-publish", kwargs={"article_id": article_id}))
        archive_response = client.post(reverse("support_desk:admin-knowledge-article-archive", kwargs={"article_id": article_id}))
...
def test_admin_article_use_records_usage_and_audit() -> None:
    ...
    admin = AdminUserFactory()
    article = publish_knowledge_article(article=create_knowledge_article(title="راهنمای پاسخ", body="متن پاسخ"))
    ...
    assert SupportKnowledgeArticleUse.objects.filter(article=article, ticket=ticket, used_by=admin).exists()
    called_actions = [call.kwargs.get("action") for call in mock_task.delay.call_args_list]
