    ...
    def test_admin_can_create_move_and_soft_delete_category_with_audit(self) -> None:
        ...
        with patch(_AUDIT_TASK_PATH) as mock_task:
            ...
            create_response = client.post(
                ...
                data={"parent_id": root.pk, "title": "زیرشاخه حرفه‌ای", "description": "برای تست", "icon": "briefcase", "order": 5},
                format="json",
            )
        ...
        delete_response = client.delete(reverse("kindness_wall:admin-category-detail", kwargs={"category_id": child_id}))
...
class TestKindnessAdminOperationalQueues:
    ...
    def test_admin_can_filter_matches_and_retrieve_detail(self) -> None:
        ...
        list_response = client.get(reverse("kindness_wall:admin-match-list"), data={"min_score": 80, "category": category.slug})
        detail_response = client.get(reverse("kindness_wall:admin-match-detail", kwargs={"match_id": match.pk}))
    ...
    def test_admin_can_list_contact_reveals_without_user_agent_bulk_noise(self) -> None:
        ...
        response = _client_for(AdminUserFactory()).get(reverse("kindness_wall:admin-contact-reveal-list"), data={"listing_id": listing.pk})
    ...
    def test_admin_can_review_duplicate_candidate_with_audit(self) -> None:
        ...
        candidate = PublishedNeedListingFactory(owner=listing.owner, category=listing.category, title="نیاز به برنامه نویس جنگو")
        duplicate = KindnessDuplicateCandidate.objects.create(listing=listing, candidate_listing=candidate, score=91)

        with patch(_AUDIT_TASK_PATH) as mock_task:
            mock_task.delay = MagicMock()
            response = _client_for(AdminUserFactory()).post(
                reverse("kindness_wall:admin-duplicate-review", kwargs={"duplicate_id": duplicate.pk}),
        ...
        assert mock_task.delay.call_args.kwargs["action"] == audit_actions.KINDNESS_DUPLICATE_REVIEWED
...
class TestKindnessAdminAnalyticsAndExports:
    ...
    def test_analytics_contains_distributions_and_match_effectiveness(self) -> None:
        ...
        KindnessMatch.objects.create(source_listing=source, target_listing=target, score=78, status="contacted")
    ...
    def test_admin_listing_export_returns_rtl_xlsx_and_audit(self) -> None:
        listing = PublishedNeedListingFactory(title="خروجی اکسل دیوار", view_count=12, contact_reveal_count=3)

        with patch(_AUDIT_TASK_PATH) as mock_task:
            mock_task.delay = MagicMock()
            response = _client_for(AdminUserFactory()).get(reverse("kindness_wall:admin-listing-export"), data={"status": ListingStatus.PUBLISHED})
        ...
        assert mock_task.delay.call_args.kwargs["action"] == audit_actions.KINDNESS_LISTINGS_EXPORTED
    ...
    def test_admin_report_export_returns_rtl_xlsx_and_audit(self) -> None:
        ...
        report = KindnessListingReport.objects.create(listing=listing, reported_by=KindnessUserFactory(), reason="other", description="بررسی شود")

        with patch(_AUDIT_TASK_PATH) as mock_task:
            mock_task.delay = MagicMock()
            response = _client_for(AdminUserFactory()).get(reverse("kindness_wall:admin-report-export"), data={"status": report.status})
