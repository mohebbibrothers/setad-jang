...
def _build_publishable_quiz(course, *, passing_score=Decimal("12.00")):
    ...
    return quiz, {q1.pk: q1_correct.pk, q2.pk: q2_correct.pk}, {q1.pk: q1_wrong.pk, q2.pk: q2_wrong.pk}
...
class TestLMSAdminQuizBuilder:
    ...
    def test_admin_can_create_quiz_question_option_and_publish(self) -> None:
        ...
        client.post(reverse("lms:admin-quiz-option-create", kwargs={"question_id": question_id}), data={"text": "غلط", "order": 1}, format="json")
        client.post(reverse("lms:admin-quiz-option-create", kwargs={"question_id": question_id}), data={"text": "درست", "order": 2, "is_correct": True}, format="json")

        publish_response = client.post(reverse("lms:admin-quiz-publish", kwargs={"course_id": course.pk}))
    ...
    def test_publish_rejects_question_without_exactly_one_correct_option(self) -> None:
        ...
        response = _client_for(admin).post(reverse("lms:admin-quiz-publish", kwargs={"course_id": course.pk}))
...
class TestLMSQuizAttemptFlow:
    ...
    def test_enrolled_user_can_start_attempt_with_question_snapshot(self) -> None:
        ...
        with patch(_AUDIT_TASK_PATH) as mock_task:
            mock_task.delay = MagicMock()
            response = _client_for(user).post(reverse("lms:quiz-attempt-start", kwargs={"slug": course.slug}))
    ...
    def test_submit_failed_attempt_hides_correct_answers(self) -> None:
        ...
        start = _client_for(user).post(reverse("lms:quiz-attempt-start", kwargs={"slug": course.slug}))
    ...
    def test_second_attempt_is_blocked_until_retake_delay(self) -> None:
        ...
        client.post(
            reverse("lms:quiz-attempt-submit", kwargs={"attempt_id": attempt_id}),
            data={"answers": [{"question_id": q, "selected_option_id": o} for q, o in wrong.items()]},
            format="json",
        )
    ...
    def test_admin_unlock_allows_extra_attempt(self) -> None:
        ...
        for _ in range(2):
            ...
            QuizAttempt.objects.filter(pk=attempt_id).update(submitted_at=timezone.now() - timezone.timedelta(days=15))
        ...
        unlocked_start = client.post(reverse("lms:quiz-attempt-start", kwargs={"slug": course.slug}))
