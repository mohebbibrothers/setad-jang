...
def _success_payment(*, authority: str = "AUTH-C6-1", ref_id: str = "REF-C6-1", amount: int = 50_000):
...
def test_admin_reconciliation_endpoints_list_detail_items_and_export() -> None:
    ...
    detail_response = client.get(reverse("madadkar:admin-reconciliation-batch-detail", kwargs={"batch_id": batch_id}))
    items_response = client.get(reverse("madadkar:admin-reconciliation-item-list", kwargs={"batch_id": batch_id}))
    export_response = client.get(reverse("madadkar:admin-reconciliation-discrepancy-export", kwargs={"batch_id": batch_id}))
