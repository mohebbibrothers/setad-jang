...
def test_metrics_path_normalization_limits_cardinality() -> None:
    ...
    assert (
        normalize_path("/api/v1/support/admin/tickets/123/") == "/api/v1/support/admin/tickets/{id}"
    )
    assert (
        normalize_path("/api/v1/items/abcdef1234567890abcdef1234567890abcdef/")
        == "/api/v1/items/{token}"
    )
...
def test_prometheus_metrics_endpoint_returns_text_exposition() -> None:
    ...
    assert (
        b"setadjang_http_requests_total" in response.content or b"python_info" in response.content
    )
