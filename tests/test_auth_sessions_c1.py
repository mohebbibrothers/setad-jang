...
def test_password_login_creates_tracked_auth_session() -> None:
    ...
    login_calls = [call.kwargs for call in mock_task.delay.call_args_list if call.kwargs.get("action") == audit_actions.LOGIN_SUCCESS]
...
def test_user_can_list_and_revoke_own_session() -> None:
    ...
    session = AuthSession.objects.get(user=user)
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {login_response.data['data']['tokens']['access']}")

    with patch(_AUDIT_TASK_PATH) as mock_task:
        mock_task.delay = MagicMock()
        list_response = client.get(reverse("authentication:session-list"))
        revoke_response = client.post(reverse("authentication:session-revoke", kwargs={"session_id": session.pk}))
...
def test_user_cannot_revoke_other_user_session() -> None:
    ...
    other_session = AuthSession.objects.create(
        ...
        fingerprint_hash=AuthSession.build_fingerprint_hash(user_agent="other", ip_address="127.0.0.1"),
    )
    client = _jwt_client(owner)

    response = client.post(reverse("authentication:session-revoke", kwargs={"session_id": other_session.pk}))
...
def test_admin_can_list_and_revoke_all_user_sessions() -> None:
    ...
    for _index in range(2):
        ...
        AuthSession.objects.create(
            ...
            fingerprint_hash=AuthSession.build_fingerprint_hash(user_agent="admin-test", ip_address="127.0.0.1"),
        )
    client = _jwt_client(admin)

    with patch(_AUDIT_TASK_PATH) as mock_task:
        mock_task.delay = MagicMock()
        list_response = client.get(reverse("authentication:admin-user-session-list", kwargs={"user_id": user.pk}))
        revoke_response = client.post(reverse("authentication:admin-user-session-revoke-all", kwargs={"user_id": user.pk}))
...
def test_logout_revokes_matching_tracked_session() -> None:
    ...
    response = client.post(reverse("authentication:logout"), data={"refresh": refresh}, format="json")
