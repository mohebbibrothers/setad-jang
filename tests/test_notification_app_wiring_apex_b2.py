...
def test_tabyin_approval_and_rejection_emit_notifications() -> None:
    ...
    assert NotificationEvent.objects.filter(event_type="tabyin.submission_approved", deliveries__recipient=approved.submitted_by).exists()
    assert NotificationEvent.objects.filter(event_type="tabyin.submission_rejected", deliveries__recipient=rejected.submitted_by).exists()
...
def test_lms_certificate_issued_emits_notification_only_for_new_skill() -> None:
    ...
    assert NotificationEvent.objects.filter(event_type="lms.certificate_issued", deliveries__recipient=certificate.user).count() == 1
...
def test_kindness_contact_reveal_and_high_match_emit_notifications() -> None:
    ...
    target = PublishedOfferListingFactory(category=source.category, title="آموزش برنامه نویسی پایتون")
    viewer = UserFactory()
    ...
    assert NotificationEvent.objects.filter(event_type="kindness.contact_revealed", deliveries__recipient=source.owner).exists()
    assert NotificationEvent.objects.filter(event_type="kindness.high_match", deliveries__recipient=source.owner).exists()
    assert NotificationTemplate.objects.filter(code="kindness.high_match", channel="in_app").exists()


@override_settings(NOTIFICATIONS_ASYNC_DISPATCH=False)
def test_madadkar_successful_payment_emits_notification() -> None:
    """Madadkar payment verification should notify paying user."""
    campaign = PublishedCampaignFactory(title="کمپین اعلان پرداخت", total_amount=100_000, total_shares=10, share_price=10_000)
    user = UserFactory()
    _participation, payment, _url = initiate_participation(campaign=campaign, user=user, share_count=1, callback_url="https://example.com/callback")

    verify_payment(authority=payment.authority)

    assert NotificationEvent.objects.filter(event_type="madadkar.payment_success", deliveries__recipient=user).exists()
