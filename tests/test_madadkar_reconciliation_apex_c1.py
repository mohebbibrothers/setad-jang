...
def _success_payment(*, authority: str = "AUTH-1", ref_id: str = "REF-1", amount: int = 100_000) -> Payment:
...
def test_reconciliation_detects_missing_internal_payment() -> None:
    ...
    batch = reconcile_provider_payments(
        provider_name="sandbox",
        rows=[{"authority": "UNKNOWN", "ref_id": "REF-MISSING", "amount": 10_000, "status": "success"}],
    )
    item = batch.items.get()
