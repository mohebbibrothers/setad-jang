...
def test_command_center_returns_cross_app_operational_summary() -> None:
    ...
    KindnessListingReport.objects.create(listing=listing, reported_by=KindnessUserFactory(), reason="other")
    ...
    Payment.objects.create(participation=participation, user=participation.user, amount=10_000, gateway_name="sandbox", authority="CMD", status=PaymentStatus.SUCCESS)

    course = PublishedCourseFactory()
    EnrollmentFactory(course=course)
    CertificateFactory(course=course)

    create_notification_event(event_type="command.event", recipients=[UserFactory()], payload={"title": "Command"})
