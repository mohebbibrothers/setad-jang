...
def test_tabyin_filter_uses_shared_search_contract() -> None:
    ...
    matched = TabyinContent.objects.create(external_id="s1", title="جهاد تبیین رسانه", description="محتوای ویژه")
    TabyinContent.objects.create(external_id="s2", title="عنوان دیگر", description="متن دیگر")

    filterset = PublicTabyinContentFilter(data={"search": "رسانه"}, queryset=TabyinContent.objects.all())
...
def test_kindness_lms_support_r4j_madadkar_filters_keep_search_behavior() -> None:
    ...
    assert list(KindnessListingPublicFilter({"search": "برنامه"}, queryset=kindness.__class__.objects.all()).qs) == [kindness]
    assert list(CoursePublicFilter({"search": "پایتون"}, queryset=course.__class__.objects.all()).qs) == [course]
    assert list(SupportUserTicketFilter({"search": "ورود"}, queryset=support.__class__.objects.filter(owner=support.owner)).qs) == [support]
    assert list(R4JCriminalPublicFilter({"search": "نمونه"}, queryset=criminal.__class__.objects.all()).qs) == [criminal]
    assert list(CampaignPublicFilter({"search": "نیازمند"}, queryset=campaign.__class__.objects.all()).qs) == [campaign]
