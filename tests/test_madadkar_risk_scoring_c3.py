...
def test_payment_failure_spike_generates_risk_signal(settings) -> None:
    ...
    assert any(signal.signal_type == MadadkarRiskSignalType.PAYMENT_FAILURE_SPIKE for signal in signals)
...
def test_suspicious_ip_velocity_generates_risk_signal(settings) -> None:
    ...
    last_payment = None
    for _index in range(3):
        _campaign, _participation, last_payment = _success_payment(amount=10_000, ip_address=ip_address)

    signals = evaluate_payment_risk(payment=last_payment)

    assert any(signal.signal_type == MadadkarRiskSignalType.SUSPICIOUS_IP_VELOCITY for signal in signals)
...
def test_large_financial_adjustment_generates_risk_signal(settings) -> None:
    ...
    applied = apply_financial_adjustment(adjustment=approve_financial_adjustment(adjustment=adjustment))

    assert applied.risk_signals.filter(signal_type=MadadkarRiskSignalType.ADJUSTMENT_ANOMALY).exists()
