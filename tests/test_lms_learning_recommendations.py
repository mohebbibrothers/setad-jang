...
def test_learning_recommendations_rank_affinity_and_level_progression() -> None:
    ...
    enrolled = PublishedCourseFactory(
        category=category, level=CourseLevel.BEGINNER, title="مقدماتی رسانه"
    )
    ...
    EnrollmentFactory(
        user=user, course=enrolled, status=EnrollmentStatus.COMPLETED, progress_percent=100
    )
...
def test_user_learning_recommendation_endpoint_returns_ranked_payload() -> None:
    ...
    PublishedCourseFactory(
        category=category, level=CourseLevel.INTERMEDIATE, title="Endpoint Recommendation"
    )
    EnrollmentFactory(
        user=user, course=enrolled, status=EnrollmentStatus.ACTIVE, progress_percent=60
    )
