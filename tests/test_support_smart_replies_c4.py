...
def test_smart_reply_suggestions_use_kb_and_exclude_internal_notes() -> None:
    ...
    SupportTicketMessageFactory(ticket=ticket, author=ticket.owner, body="پرداخت من خطا دارد", is_internal=False)
...
def test_smart_reply_suggestions_include_canned_response_and_fallback() -> None:
    ...
    source_pairs = {(suggestion.source_type, suggestion.source_id) for suggestion in bundle.suggestions}
...
def test_smart_reply_endpoint_audits_generation() -> None:
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        response = client.get(reverse("support_desk:admin-ticket-smart-replies", kwargs={"ticket_number": ticket.ticket_number}))
...
def test_smart_reply_use_sends_reply_records_kb_use_and_audit() -> None:
    ...
    admin = AdminUserFactory()
    ticket, department, category, ticket_type = _ticket_with_taxonomy(status_value=TicketStatus.WAITING_FOR_ADMIN)
    article = create_knowledge_article(
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        response = client.post(
            reverse("support_desk:admin-ticket-smart-reply-use", kwargs={"ticket_number": ticket.ticket_number}),
            data={
    ...
    assert SupportTicketMessage.objects.filter(ticket=ticket, is_from_staff=True, body__icontains="شماره تراکنش").exists()
    assert SupportKnowledgeArticleUse.objects.filter(article=article, ticket=ticket, used_by=admin, context="smart_reply").exists()
    called_actions = [call.kwargs.get("action") for call in mock_task.delay.call_args_list]
