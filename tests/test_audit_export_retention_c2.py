...
def test_forensic_export_package_contains_manifest_jsonl_csv_and_xlsx() -> None:
    ...
    assert manifest["chain_verification"]["head_hash"] == AuditLog.objects.order_by("-created_at", "-id").first().event_hash
    assert "retention_policy" in manifest

    jsonl_rows = [json.loads(line) for line in files["audit_logs.jsonl"].decode("utf-8").splitlines()]
...
def test_admin_export_endpoint_returns_zip_and_logs_export() -> None:
    ...
    manifest = json.loads(files["manifest.json"].decode("utf-8"))
    assert manifest["filters"]["action"] == LOGIN_SUCCESS
    assert AuditLog.objects.filter(action=AUDIT_PACKAGE_EXPORTED, request_id="audit-export-req-1").exists()
