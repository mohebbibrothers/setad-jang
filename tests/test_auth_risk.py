...
def test_password_login_generates_auth_risk_signals_for_new_device() -> None:
    ...
    assert AuthRiskSignal.objects.filter(
        user=user, session=session, signal_type=AuthRiskSignalType.NEW_DEVICE
    ).exists()
    assert AuthRiskSignal.objects.filter(
        user=user, session=session, signal_type=AuthRiskSignalType.NEW_IP
    ).exists()
