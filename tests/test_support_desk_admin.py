...
class TestSupportAdminTaxonomyAPI:
    ...
    def test_admin_can_create_update_and_deactivate_department_with_audit(self) -> None:
        ...
        assert (
            mock_task.delay.call_args.kwargs["action"] == audit_actions.SUPPORT_DEPARTMENT_CREATED
        )

        update_response = client.patch(
            reverse(
                "support_desk:admin-department-detail", kwargs={"department_id": department_id}
            ),
    ...
    def test_admin_can_manage_ticket_type_sla_policy_and_canned_response(self) -> None:
        ...
        type_response = client.post(
            ...
            data={
                "code": "ops-special",
                "title": "عملیات ویژه",
                "default_department_id": department.pk,
                "default_category_id": category.pk,
                "default_sla_policy_id": sla_id,
        ...
        use_response = client.post(
            reverse(
                "support_desk:admin-canned-response-use", kwargs={"canned_response_id": canned_id}
            )
        )
...
class TestSupportAdminTicketOperationsAPI:
    ...
    def test_admin_queue_detail_reply_internal_note_assign_status_and_escalate(self) -> None:
        ...
        list_response = client.get(
            reverse("support_desk:admin-ticket-list"), data={"status": TicketStatus.SUBMITTED}
        )
        ...
        reply_response = client.post(
            reverse(
                "support_desk:admin-ticket-reply", kwargs={"ticket_number": ticket.ticket_number}
            ),
        ...
        note_response = client.post(
            reverse(
                "support_desk:admin-ticket-internal-note",
        ...
        assign_response = client.post(
            reverse(
                "support_desk:admin-ticket-assign", kwargs={"ticket_number": ticket.ticket_number}
            ),
            data={
        ...
        status_response = client.post(
            reverse(
                "support_desk:admin-ticket-status", kwargs={"ticket_number": ticket.ticket_number}
            ),
        ...
        escalate_response = client.post(
            reverse(
                "support_desk:admin-ticket-escalate", kwargs={"ticket_number": ticket.ticket_number}
            ),
            data={"reason": "فوری"},
            format="json",
        )
        ...
        detail_response = client.get(
            reverse(
                "support_desk:admin-ticket-detail", kwargs={"ticket_number": ticket.ticket_number}
            )
        )
    ...
    def test_admin_can_review_duplicate_candidate(self) -> None:
        ...
        with patch(_AUDIT_TASK_PATH) as mock_task:
            ...
            response = _client_for(AdminUserFactory()).post(
                reverse(
                    "support_desk:admin-duplicate-review", kwargs={"duplicate_id": duplicate.pk}
                ),
        ...
        assert (
            mock_task.delay.call_args.kwargs["action"] == audit_actions.SUPPORT_DUPLICATE_REVIEWED
        )
    ...
    def test_department_with_open_ticket_cannot_be_deactivated(self) -> None:
        ...
        ticket = create_ticket(
            ...
            ticket_type=ticket_type,
            subject="باز",
            description="تیکت باز برای guard",
        )
        submit_ticket(ticket=ticket, user=ticket.owner)

        response = _client_for(AdminUserFactory()).delete(
            reverse(
                "support_desk:admin-department-detail",
                kwargs={"department_id": ticket.department_id},
            )
        )
    ...
    def test_admin_close_endpoint_closes_resolved_ticket(self) -> None:
        ...
        response = _client_for(AdminUserFactory()).post(
            reverse(
                "support_desk:admin-ticket-close", kwargs={"ticket_number": ticket.ticket_number}
            ),
        ...
        assert SupportTicketMessage.objects.filter(
            ticket__ticket_number=ticket.ticket_number, message_type="status_change"
        ).exists()
