...
def test_audit_logs_receive_previous_and_event_hashes() -> None:
    ...
    first = create_audit_log(
        user_id=user.pk, action=LOGIN_SUCCESS, resource_type="user", resource_id=str(user.pk)
    )
    second = create_audit_log(
        user_id=user.pk, action=LOGOUT, resource_type="user", resource_id=str(user.pk)
    )
