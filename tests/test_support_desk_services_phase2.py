...
class TestSupportTicketCreationAndSubmit:
    ...
    def test_create_ticket_uses_dynamic_defaults_and_creates_timeline_tags_duplicates(self) -> None:
        ...
        assert ticket.messages.filter(message_type=TicketMessageType.USER_MESSAGE, is_internal=False).count() == 1
        assert ticket.message_count >= 1
        assert SupportTicketStatusHistory.objects.filter(ticket=ticket, to_status=TicketStatus.DRAFT).exists()
    ...
    def test_submit_ticket_applies_sla_and_changes_status(self) -> None:
        ticket_type, _department, _category, policy = _make_type_with_policy(first_minutes=30, resolution_minutes=90)
        owner = UserFactory()
        now = timezone.now()
        ticket = create_ticket(owner=owner, ticket_type=ticket_type, subject="مشکل فنی", description="صفحه خطا می‌دهد")
        ...
        assert SupportSLAEvent.objects.filter(ticket=ticket, event_type=SLAEventType.POLICY_APPLIED).exists()
...
class TestSupportConversationWorkflow:
    ...
    def test_admin_reply_sets_first_response_waiting_for_user_and_pauses_sla(self) -> None:
        ticket_type, _department, _category, _policy = _make_type_with_policy(first_minutes=30, resolution_minutes=90)
        owner = UserFactory()
        admin = AdminUserFactory()
        now = timezone.now()
        ticket = submit_ticket(ticket=create_ticket(owner=owner, ticket_type=ticket_type, subject="پرداخت", description="رسید ثبت نشده"), user=owner, now=now)

        message = add_admin_reply(ticket=ticket, admin=admin, body="لطفاً شماره پیگیری را ارسال کنید", now=now + timezone.timedelta(minutes=10))
        ...
        assert SupportSLAEvent.objects.filter(ticket=ticket, event_type=SLAEventType.PAUSED).exists()
    ...
    def test_user_reply_resumes_sla_and_moves_to_waiting_for_admin(self) -> None:
        ...
        now = timezone.now()
        ticket = submit_ticket(ticket=create_ticket(owner=owner, ticket_type=ticket_type, subject="پرداخت", description="رسید ثبت نشده"), user=owner, now=now)
        add_admin_reply(ticket=ticket, admin=admin, body="شماره پیگیری؟", now=now + timezone.timedelta(minutes=5))
        ticket.refresh_from_db()
        original_resolution_due = ticket.resolution_due_at

        add_user_reply(ticket=ticket, user=owner, body="شماره پیگیری ۱۲۳۴", now=now + timezone.timedelta(minutes=65))
        ticket.refresh_from_db()
        ...
        assert SupportSLAEvent.objects.filter(ticket=ticket, event_type=SLAEventType.RESUMED).exists()
    ...
    def test_assignment_records_history_and_internal_timeline(self) -> None:
        ...
        assign_ticket(ticket=ticket, admin=admin, assignee=assignee, department=department, reason="نیاز به بررسی تخصصی")
        ticket.refresh_from_db()
        ...
        assert SupportTicketMessage.objects.filter(ticket=ticket, message_type=TicketMessageType.ASSIGNMENT_CHANGE, is_internal=True).exists()
...
class TestSupportStatusResolutionAndSatisfaction:
    ...
    def test_resolve_close_reopen_and_satisfaction_flow(self) -> None:
        ...
        satisfaction = submit_satisfaction(ticket=ticket, user=ticket.owner, rating=5, comment="عالی")
        ticket.refresh_from_db()
    ...
    def test_invalid_status_and_non_admin_status_change_are_rejected(self) -> None:
        ...
        with pytest.raises(SupportPermissionError):
            change_ticket_status(ticket=ticket, admin=UserFactory(), status=TicketStatus.IN_PROGRESS)
...
class TestSupportSLAAndEscalation:
    ...
    def test_mark_sla_breaches_marks_and_escalates_ticket(self) -> None:
        ticket_type, _department, _category, _policy = _make_type_with_policy(first_minutes=1, resolution_minutes=2)
        owner = UserFactory()
        now = timezone.now()
        ticket = submit_ticket(ticket=create_ticket(owner=owner, ticket_type=ticket_type, subject="بحرانی", description="خطای مهم"), user=owner, now=now)
        ...
        assert SupportSLAEvent.objects.filter(ticket=ticket, event_type=SLAEventType.ESCALATED).exists()
...
class TestSupportSmartTriageAndDuplicates:
    ...
    def test_smart_triage_suggests_payment_priority_and_similar_tickets(self) -> None:
        ...
        existing = create_ticket(owner=owner, ticket_type=payment_type, subject="پرداخت ثبت نشده", description="پرداخت انجام شده اما ثبت نشده")

        suggestion = suggest_ticket_triage(owner=owner, subject="پرداخت ثبت نشده", description="پرداخت انجام شده اما ثبت نشده")
    ...
    def test_duplicate_detection_and_review(self) -> None:
        ...
        reviewed = review_duplicate_candidate(duplicate=duplicate, admin=AdminUserFactory(), status=DuplicateReviewStatus.CONFIRMED, reason="تکراری است")
...
class TestSupportTaxonomyServices:
    ...
    def test_category_create_move_cycle_and_deactivate_guards(self) -> None:
        ...
        ticket_type = SupportTicketTypeFactory(default_department=department, default_category=child)
        ticket = create_ticket(owner=UserFactory(), ticket_type=ticket_type, subject="باز", description="تیکت باز")
