...
def _make_sla_ticket(*, breached: bool = False) -> SupportTicket:
    ...
    policy = SupportSLAPolicyFactory(title="SLA گزارش", first_response_minutes=1, resolution_minutes=2)
    ticket_type = SupportTicketTypeFactory(default_sla_policy=policy)
    ticket = create_ticket(owner=UserFactory(), ticket_type=ticket_type, subject="گزارش پرداخت", description="پرداخت انجام شد اما ثبت نشد")
...
class TestSupportAdminAnalytics:
    ...
    def test_admin_analytics_contains_rates_distributions_and_csat(self) -> None:
        ...
        SupportTicketSatisfaction.objects.create(ticket=ticket, user=ticket.owner, rating=5, comment="عالی")
...
class TestSupportAdminExports:
    ...
    def test_ticket_export_returns_rtl_xlsx_and_dispatches_audit(self) -> None:
        ...
        with patch(_AUDIT_TASK_PATH) as mock_task:
            ...
            response = _client_for(admin).get(reverse("support_desk:admin-export-tickets"), data={"search": ticket.ticket_number})
...
class TestSupportTasks:
    ...
    def test_cleanup_stale_support_drafts_task_archives_old_drafts(self) -> None:
        ...
        SupportTicket.objects.filter(pk=old_ticket.pk).update(created_at=timezone.now() - timezone.timedelta(days=45))
