...
class TestLMSLessonAnswersAndAccept:
    ...
    def test_enrolled_user_can_answer_question(self) -> None:
        ...
        question = LessonQuestion.objects.create(lesson=lesson, user=owner, title="سؤال", body="متن سؤال معتبر")
...
class TestLMSDiscussionReportsAndModeration:
    ...
    def test_admin_can_hide_question_and_review_report(self) -> None:
        ...
        _enroll(user, course)
        question = LessonQuestion.objects.create(lesson=lesson, user=user, title="سؤال", body="متن سؤال معتبر")
        report = LessonDiscussionReport.objects.create(question=question, reported_by=user, reason="spam")
