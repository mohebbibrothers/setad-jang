...
class TestKindnessListingOwnerWorkflow:
    ...
    def test_sensitive_update_moves_published_listing_back_to_pending_review(self) -> None:
        ...
        updated = update_listing(listing=listing, user=listing.owner, title="برنامه نویس فول استک نیاز دارم")
...
class TestKindnessMatchAndMaintenance:
    ...
    def test_match_can_be_dismissed_and_marked_contacted_by_source_owner(self) -> None:
        source = PublishedNeedListingFactory(title="برنامه نویس لازم دارم", description="طراحی سایت")
        target = PublishedOfferListingFactory(category=source.category, title="برنامه نویس هستم", description="طراحی سایت")
    ...
    def test_expire_due_listings_and_analytics_summary(self) -> None:
        expired = PublishedNeedListingFactory(expires_at=timezone.now() - timezone.timedelta(days=1))
        active = PublishedOfferListingFactory(expires_at=timezone.now() + timezone.timedelta(days=10))
    ...
    def test_duplicate_candidate_review_changes_status(self) -> None:
        ...
        review_duplicate_candidate(duplicate=duplicate, status=DuplicateStatus.CONFIRMED, reason="واقعاً تکراری است")
