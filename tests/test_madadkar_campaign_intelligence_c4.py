...
def test_campaign_intelligence_computes_refund_adjusted_financials_and_health() -> None:
    ...
    FailedPaymentFactory(participation=PaidParticipationFactory(campaign=campaign, total_amount=5_000), amount=5_000)
    refund = request_payment_refund(payment=first_payment, amount=5_000, idempotency_key="intel-refund-1")
...
def test_admin_campaign_intelligence_endpoint_returns_payload() -> None:
    ...
    response = client.get(reverse("madadkar:admin-campaign-intelligence", kwargs={"campaign_id": campaign.pk}), data={"days": 5})
