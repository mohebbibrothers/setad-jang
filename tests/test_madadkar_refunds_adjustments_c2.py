...
def _successful_payment(*, amount: int = 20_000):
    ...
    payment = SuccessPaymentFactory(participation=participation, amount=amount, user=participation.user)
    campaign.purchased_shares = participation.share_count
    campaign.purchased_amount = amount
    campaign.participant_count = 1
    campaign.save(update_fields=["purchased_shares", "purchased_amount", "participant_count", "updated_at"])
...
def test_full_refund_releases_shares_and_marks_participation_refunded() -> None:
    ...
    refund = request_payment_refund(payment=payment, amount=20_000, reason=RefundReason.ADMIN_CORRECTION)
...
def test_admin_refund_api_creates_approves_and_completes_with_audit() -> None:
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        approve_response = client.post(reverse("madadkar:admin-refund-action", kwargs={"refund_id": refund_id, "action": "approve"}))
        complete_response = client.post(
            reverse("madadkar:admin-refund-action", kwargs={"refund_id": refund_id, "action": "complete"}),
...
def test_admin_adjustment_api_applies_with_audit() -> None:
    ...
    with patch(_AUDIT_TASK_PATH) as mock_task:
        ...
        approve_response = client.post(reverse("madadkar:admin-adjustment-action", kwargs={"adjustment_id": adjustment_id, "action": "approve"}))
        apply_response = client.post(reverse("madadkar:admin-adjustment-action", kwargs={"adjustment_id": adjustment_id, "action": "apply"}))

    assert create_response.status_code == status.HTTP_201_CREATED
    assert approve_response.status_code == status.HTTP_200_OK
    assert apply_response.status_code == status.HTTP_200_OK
    assert CampaignFinancialAdjustment.objects.get(pk=adjustment_id).status == FinancialAdjustmentStatus.APPLIED
    called_actions = [call.kwargs.get("action") for call in mock_task.delay.call_args_list]
