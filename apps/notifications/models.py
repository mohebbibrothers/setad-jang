...
class NotificationEvent(BaseModel):
    ...
    actor = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="notification_events_created")
    ...
    status = models.CharField(max_length=20, choices=NotificationEventStatus.choices, default=NotificationEventStatus.PENDING, db_index=True)
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [models.Index(fields=["status", "next_retry_at", "-created_at"]), models.Index(fields=["event_type", "-created_at"])]
...
class NotificationDelivery(BaseModel):
    ...
    channel = models.CharField(max_length=20, choices=NotificationChannel.choices)
    status = models.CharField(max_length=20, choices=NotificationDeliveryStatus.choices, default=NotificationDeliveryStatus.PENDING, db_index=True)
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [models.Index(fields=["recipient", "channel", "status", "-created_at"]), models.Index(fields=["event", "channel"])]
        constraints = [models.UniqueConstraint(fields=["event", "recipient", "channel"], name="uniq_notification_delivery_event_recipient_channel")]
...
class NotificationPreference(BaseModel):
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["user", "event_type", "channel"], name="uniq_notification_preference")]
