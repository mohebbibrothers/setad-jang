...
class PrometheusMetricsView(APIView):
    ...
    @extend_schema(operation_id="metrics_prometheus", tags=["Observability"], responses={200: None, 404: None})
    def get(self, request):
        """Return Prometheus text exposition format."""
        if not getattr(settings, "PROMETHEUS_METRICS_ENABLED", True):
            return HttpResponse("metrics disabled", status=404, content_type="text/plain; charset=utf-8")
