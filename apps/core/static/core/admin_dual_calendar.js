...
(function () {
  ...
  function fieldContainer(input) {
    return input.closest(".fieldBox, .form-row, td, .inline-related") || input.parentElement || document;
  }

  function wrapperBelongsToInput(wrapper, input) {
    const identity = inputIdentity(input);
    if (identity && wrapper.dataset.sjDateInput === identity) return true;

    // Backward-compatible cleanup for wrappers generated by older versions that
    // did not have a stable input marker. Scope the search to the smallest admin
    // field container so different date fields in the same inline are not touched.
    return fieldContainer(input).contains(wrapper);
  }
  ...
  function removeCalendarController(input) {
    ...
    fieldContainer(input).querySelectorAll(".sj-date-enhanced").forEach((wrapper) => {
      if (wrapperBelongsToInput(wrapper, input)) wrapper.remove();
    });
  }
  ...
  function enhanceDateInput(input) {
    ...
    if (existingController && existingController.button && existingController.button.isConnected) {
      const activeWrapper = existingController.button.closest(".sj-date-enhanced");
      fieldContainer(input).querySelectorAll(".sj-date-enhanced").forEach((wrapper) => {
        if (wrapper !== activeWrapper && wrapperBelongsToInput(wrapper, input)) wrapper.remove();
      });
      return;
    }
    ...
    wrapper.dataset.sjDateInput = inputIdentity(input);
