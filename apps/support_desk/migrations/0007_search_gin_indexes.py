...
ایندکس‌های expression جستجو برای جدول `support_desk_supportticket`:
...
_INDEX_NAMES = [
    "support_desk_supportticket_search_fts_idx",
    "support_desk_supportticket_search_trgm_ticket_number",
    "support_desk_supportticket_search_trgm_subject",
]
...
def build_indexes(apps, schema_editor) -> None:
    ...
    model = apps.get_model("support_desk", "SupportTicket")
    statements = [
        fts_index_sql(
            model,
            index_name="support_desk_supportticket_search_fts_idx",
            fields=[
        ...
        trigram_index_sql(
            model,
            index_name="support_desk_supportticket_search_trgm_ticket_number",
            field_name="ticket_number",
        ),
        trigram_index_sql(
            model, index_name="support_desk_supportticket_search_trgm_subject", field_name="subject"
        ),
    ]
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("support_desk", "0006_supportknowledgearticle_supportknowledgearticleuse_and_more")
    ]
