...
ایندکس‌های expression جستجو برای جدول `madadkar_campaign`:
...
_INDEX_NAMES = [
    "madadkar_campaign_search_fts_idx",
    "madadkar_campaign_search_trgm_title",
    "madadkar_campaign_search_trgm_description",
    "madadkar_campaign_search_trgm_name",
]
...
def build_indexes(apps, schema_editor) -> None:
    ...
    model = apps.get_model("madadkar", "Campaign")
    model_sponsor = apps.get_model("madadkar", "Sponsor")
    statements = [
        fts_index_sql(
            model,
            index_name="madadkar_campaign_search_fts_idx",
            fields=[
                SearchField("title", "A"),
        ...
        trigram_index_sql(
            model, index_name="madadkar_campaign_search_trgm_title", field_name="title"
        ),
        trigram_index_sql(
            model, index_name="madadkar_campaign_search_trgm_description", field_name="description"
        ),
        trigram_index_sql(
            model_sponsor, index_name="madadkar_campaign_search_trgm_name", field_name="name"
        ),
    ]
...
class Migration(migrations.Migration):
    ...
    dependencies = [("madadkar", "0008_madadkarfinancialcontrolsnapshot")]
