...
ایندکس‌های expression جستجو برای جدول `lms_course`:
...
_INDEX_NAMES = [
    "lms_course_search_fts_idx",
    "lms_course_search_trgm_title",
    "lms_course_search_trgm_subtitle",
    "lms_course_search_trgm_instructor_name",
]
...
def build_indexes(apps, schema_editor) -> None:
    ...
    statements = [
        fts_index_sql(
            model,
            index_name="lms_course_search_fts_idx",
            fields=[
                SearchField("title", "A"),
        ...
        trigram_index_sql(model, index_name="lms_course_search_trgm_title", field_name="title"),
        trigram_index_sql(
            model, index_name="lms_course_search_trgm_subtitle", field_name="subtitle"
        ),
        trigram_index_sql(
            model, index_name="lms_course_search_trgm_instructor_name", field_name="instructor_name"
        ),
    ]
...
class Migration(migrations.Migration):
    ...
    dependencies = [("lms", "0004_learningactivitystatement")]
