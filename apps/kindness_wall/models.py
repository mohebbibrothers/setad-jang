...
class KindnessCategory(BaseModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["parent", "title"], name="uniq_kindness_category_parent_title"),
        ]
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not self.slug:
            self.slug = _unique_slug(model=KindnessCategory, value=self.title, max_length=220, exclude_pk=self.pk)
        if self.parent_id:
            parent = self.parent
...
class KindnessTag(BaseModel):
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not self.slug:
            self.slug = _unique_slug(model=KindnessTag, value=self.normalized_name, max_length=160, exclude_pk=self.pk)
        super().save(*args, **kwargs)
...
class KindnessKeywordAlias(BaseModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["normalized_keyword", "normalized_alias"], name="uniq_kindness_keyword_alias"),
        ]
...
class KindnessListing(BaseModel):
    ...
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="kindness_listings")
    listing_type = models.CharField(max_length=20, choices=ListingType.choices, db_index=True)
    category = models.ForeignKey(KindnessCategory, on_delete=models.PROTECT, related_name="listings")
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not self.slug:
            self.slug = _unique_slug(model=KindnessListing, value=self.title, max_length=320, exclude_pk=self.pk)
        self.search_document = f"{self.title} {self.description} {self.province} {self.city} {self.category.title}"
        super().save(*args, **kwargs)
...
class KindnessListingImage(BaseModel):
    ...
    image_kind = models.CharField(max_length=20, choices=ListingImageKind.choices, default=ListingImageKind.GALLERY)
...
class KindnessListingTag(BaseModel):
    ...
    listing = models.ForeignKey(KindnessListing, on_delete=models.CASCADE, related_name="listing_tags")
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["listing", "tag"], name="uniq_kindness_listing_tag")]
        indexes = [models.Index(fields=["tag", "weight"]), models.Index(fields=["listing", "source"])]


class KindnessMatch(BaseModel):
    """Materialized smart match between opposite listing types."""

    source_listing = models.ForeignKey(KindnessListing, on_delete=models.CASCADE, related_name="outgoing_matches")
    target_listing = models.ForeignKey(KindnessListing, on_delete=models.CASCADE, related_name="incoming_matches")
    score = models.PositiveSmallIntegerField(validators=[validate_match_score])
    score_breakdown = models.JSONField(default=dict, blank=True)
    reason_codes = models.JSONField(default=list, blank=True)
    explanation = models.TextField(blank=True)
    status = models.CharField(max_length=20, choices=MatchStatus.choices, default=MatchStatus.ACTIVE, db_index=True)
    algorithm_version = models.PositiveIntegerField(default=1)
    generated_at = models.DateTimeField(default=timezone.now)
    dismissed_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True)
    dismissed_at = models.DateTimeField(null=True, blank=True)
    contacted_at = models.DateTimeField(null=True, blank=True)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["source_listing", "target_listing"], name="uniq_kindness_match_pair"),
            models.CheckConstraint(name="kindness_match_not_self", condition=~models.Q(source_listing=models.F("target_listing"))),
        ]
        indexes = [models.Index(fields=["source_listing", "status", "-score"]), models.Index(fields=["target_listing", "status", "-score"])]


class KindnessContactReveal(BaseModel):
    """Audit/safety record for revealing a listing owner's phone number."""

    listing = models.ForeignKey(KindnessListing, on_delete=models.PROTECT, related_name="contact_reveals")
    viewer = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="kindness_contact_reveals")
    listing_owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="kindness_received_contact_reveals")
    ...
    class Meta:
        indexes = [models.Index(fields=["listing", "-created_at"]), models.Index(fields=["viewer", "-created_at"])]
...
class KindnessListingReport(BaseModel):
    ...
    reported_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="kindness_reports")
    ...
    reviewed_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="kindness_reviewed_reports")
    reviewed_at = models.DateTimeField(null=True, blank=True)
    admin_note = models.TextField(blank=True)

    class Meta:
        indexes = [models.Index(fields=["status", "-created_at"]), models.Index(fields=["listing", "status"])]


class KindnessBookmark(BaseModel):
    """User bookmark for a published listing."""

    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="kindness_bookmarks")
...
class KindnessDuplicateCandidate(BaseModel):
    ...
    listing = models.ForeignKey(KindnessListing, on_delete=models.CASCADE, related_name="duplicate_candidates")
    candidate_listing = models.ForeignKey(KindnessListing, on_delete=models.CASCADE, related_name="duplicate_of_candidates")
    score = models.PositiveSmallIntegerField(validators=[validate_match_score])
    reason = models.TextField(blank=True)
    status = models.CharField(max_length=20, choices=DuplicateStatus.choices, default=DuplicateStatus.ACTIVE)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["listing", "candidate_listing"], name="uniq_kindness_duplicate_candidate"),
            models.CheckConstraint(name="kindness_duplicate_not_self", condition=~models.Q(listing=models.F("candidate_listing"))),
        ]
